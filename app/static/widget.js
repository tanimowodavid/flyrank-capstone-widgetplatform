...
(function () {
  ...
  function getWidgetIdFromScript() {
    ...
    if (!script) {
      console.warn('[FlyRank Widget] Unable to locate script tag (document.currentScript not available)');
      return null;
    }
  ...
  async function fetchWidgetConfig(widgetId, baseUrl) {
    try {
      var response = await fetch(baseUrl + '/api/v1/widgets/' + widgetId + '/config', {
        method: 'GET',
      ...
      if (!response.ok) {
        console.warn('[FlyRank Widget] Failed to fetch config: ' + response.status);
        return null;
      }
